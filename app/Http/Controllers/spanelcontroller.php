...
class spanelcontroller extends Controller
{
  ...
  public function competition()
  {
    ...
    $session_user= Session::get('superadmin');
    $user=website_admins::where('email',$session_user)->get()->first();
    $notifications_all=Notifications::where('website_accounts_id',999999999)->orderBy('id','desc')->take(50)->get()->all();
    $notifications_unseen=Notifications::where('website_accounts_id',999999999)->where('notification_status',0)->get()->all();
    //$accounts=Competetion::whereDate('created_at', Carbon::today())->get()->all();
    $accounts=Competetion::whereDate('created_at', Carbon::today())->get()->all();

        if( Request::segment(1) =='ar'){
            $title = "لوحة التحكم | الأدارة ";
            return view('ar.spanel.competition',compact('title','user','notifications_all','notifications_unseen','accounts'));
        }else{
            $title = "Spanel | Home";
            return view('spanel.competition',compact('title','user','notifications_all','notifications_unseen','accounts'));
        }


  }

  public function update_competition($login)
  {
    $account=Competetion::where('account_id',$login)->get()->first();
    if($account->lots==0){$account->lots=1;}else{$account->lots=0;}
    $account->save();
return redirect()->back();
    ...
    public function login()
    {
        ...
        if (Session::has('suerpanel')) {

            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/home');
            }else{
                return redirect('en/spanel/home');
            }
            ...
            if( Request::segment(1) =='ar'){
                $title = "تسجيل الدخول";
                return view('ar.spanel.login',compact('title'));
            }else{
                $title = "Client Login";
                return view('spanel.login',compact('title'));
            }
    ...
    function checklogin()
     {
        $input = Request::all();
        $user = Website_admins::where('email', $input['username'])->get()->first();



         if ($user) {
             if (Hash::check($input['password'], $user->password)) {
                 Session::put('superadmin', $input['username']);
                 Session::save();
    ...
    public function post_password()
    {
        ...
        $user=website_admins::where('email',$session_user)->get()->first();
              ...
              if(Hash::check($input['currentpassword'], $user->password)) {
	            ...
	            if( Request::segment(1) =='ar'){
	                return redirect('ar/spanel/home')->with('status-success', 'تم تغيير كلمة السر');
	            }else{
	                return redirect('en/spanel/home')->with('status-success', 'Password updated!');
	            }
                ...
                if( Request::segment(1) =='ar'){
                    return redirect('ar/spanel/home')->with('status-error', 'كلمة السر الحالية غير صحيحة');
                }else{
                    return redirect('en/spanel/home')->with('status-error', 'Current Password Wrong !');
                }
        ...
        public function post_User_Password()
        {
            ...
            $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    $user_password_change=website_accounts::where('email',$input['email'])->get();
                    if(count($user_password_change)>0){
                      $user_password_change=website_accounts::where('email',$input['email'])->get()->first();

      	            $encpassword= Hash::make($input['password']);

      	            $user_password_change->password=$encpassword;
      	            if( $user_password_change->save() ){

      	            if( Request::segment(1) =='ar'){
      	                return redirect('ar/spanel/home')->with('status-success', 'تم تغيير كلمة السر');
      	            }else{
      	                return redirect('en/spanel/home')->with('status-success', 'User Password updated!');
      	            }

      	            }
                    }else{
                      if( Request::segment(1) =='ar'){
                          return redirect('ar/spanel/home')->with('status-error', 'تم تغيير كلمة السر');
                      }else{
                          return redirect('en/spanel/home')->with('status-error', 'Wrong User Email!');
                      }
                    }
    ...
    public function open_live_account()
    {
        ...
        $user=website_admins::where('email',$session_user)->get()->first();
            ...
            $fx_account->account_id=$input['mt4login'];
            ...
            $fx_account->account_radio_type=1;
            $fx_account->password=$input['mt4password'];
            $fx_account->investor_password=$input['mt4investor'];
            $fx_account->website_accounts_id=$input['user_id'];
            $fx_account->status=0;
                if( $fx_account->save() ){

                $notification=new Notifications;
                $notification->website_accounts_id=$input['user_id'];
                $notification->notification_status=0;
                $notification->notification='Live Account '.$input['mt4login'].' Has Been Opened Successfully';
                $notification->details='Live Account '.$input['mt4login'].' Has Been Opened Successfully';
                $notification->notification_ar='حساب حقيقى '.$input['mt4login'].' تم انشائه بنجاح';
                $notification->details_ar='حساب حقيقى '.$input['mt4login'].' تم انشائه بنجاح';

                $notification->notification_ru='Реальный счет '.$input['mt4login'].' успешно открыт';
                $notification->details_ru='Реальный счет '.$input['mt4login'].' успешно открыт';

                $notification->notification_link='/cpanel/live_accounts';
                $notification->save();

                if( Request::segment(1) =='ar'){
                    return redirect('ar/spanel/home')->with('status-success', 'تم تغيير كلمة السر');
                }else{
                    return redirect('en/spanel/home')->with('status-success', 'Account Added Successfully!');
                }
    ...
    public function edit_live_account()
    {
       ...
       $account=fx_accounts::where('id',$input['account_id'])->where('account_id',$input['mt4login'])->where('website_accounts_id',$input['user_id'])->get()->first();
       if($account == Null){
         if( Request::segment(1) =='ar'){
           return redirect('ar/spanel/home')->with('status-error', 'Account Invalid');
         }else{
             return redirect('en/spanel/home')->with('status-error', 'Account Invalid');
         }
       }
        $account->leverage=$input['leverage'];
        $account->account_group=$input['account_group'];
        $account->account_type=$input['account_type'];

               $notification=new Notifications;
                $notification->website_accounts_id=$input['user_id'];
                $notification->notification_status=0;
                $notification->notification='Account '.$input['mt4login'].' has been edited as your request.';
                $notification->details='Account '.$input['mt4login'].' has been edited as your request.';
                $notification->notification_ar='تم تغيير حساب رقم  '.$input['mt4login'].'بنجاح بناءا على طلبك';
                $notification->details_ar=' تم تعديلة بنجاح '.$input['mt4login'].' حساب رقم ';
                $notification->notification_ru='Account '.$input['mt4login'].' has been edited as your request.';
                $notification->details_ru='Аккаунт '.$input['mt4login'].' отредактирован по вашему запросу.';
                ...
                if( Request::segment(1) =='ar'){
                    ...
                    return redirect('en/spanel/home')->with('status-success', 'Account Updated Successfully!');
                }
   ...
   public function home()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',999999999)->where('notification_status',0)->get()->all();
            ...
            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | الأدارة ";
                return view('ar.spanel.home',compact('title','user','notifications_all','notifications_unseen'));
            }else{
                $title = "Spanel | Home";
                return view('spanel.home',compact('title','user','notifications_all','notifications_unseen'));
            }
   ...
   public function admins()
    {
        ...
        $admins=website_admins::get()->all();
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.admins',compact('title','user','notifications_all','notifications_unseen','admins'));
            }else{
                $title = "Spanel | Admins";
                return view('spanel.admins',compact('title','user','notifications_all','notifications_unseen','admins'));
            }
    ...
    public function add_admins()
    {
        ...
        $user=website_admins::where('email',$session_user)->get()->first();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/admins');
            }else{
                return redirect('en/spanel/admins');
            }
    ...
    public function edit_admins($id)
    {
        ...
        Session::flash('pageType', 'general');
        Session::flash('currentPage', 'admins');
        $session_user= Session::get('superadmins');
    ...
    public function website_accounts()
    {
        ...
        $live_accounts=Fx_accounts::where('account_radio_type',1)->get()->all();
            ...
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.website-accounts',compact('title','user','notifications_all','notifications_unseen','website_accounts','documents','demo_accounts','live_accounts'));
            }else{
                $title = "Spanel | Website Accounts";
                return view('spanel.website-accounts',compact('title','user','notifications_all','notifications_unseen','website_accounts','documents','demo_accounts','live_accounts'));
            }
    ...
    public function demo_accounts()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',999999999)->where('notification_status',0)->get()->all();
        $user=website_admins::where('email',$session_user)->get()->first();
        $accounts=Fx_accounts::where('account_radio_type',0)->orderBy('id','desc')->paginate(10);
        ...
        foreach($accounts as $account){
            ...
            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | الحسابات الديمو ";
                return view('ar.spanel.demo-accounts',compact('title','user','notifications_all','notifications_unseen','accounts','balances','equities'));
            }else{
                $title = "Spanel | Demo Accounts";
                return view('spanel.demo-accounts',compact('title','user','notifications_all','notifications_unseen','accounts','balances','equities'));
            }
    ...
    public function delete_demo_accounts($id)
    {
        ...
        $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    $demo_account=Fx_accounts::where('id',$id)->get()->first();
                        ...
                        $notification=new Notifications;
                        $notification->website_accounts_id=$demo_account->website_accounts_id;
                        $notification->notification_status=0;
                        $notification->notification='Account '.$demo_account->account_id.' Has Been Deleted Successfully';
                        $notification->details='Account '.$demo_account->account_id.' Has Been Deleted Successfully';

                        $notification->notification_ar='تم حذف حساب رقم '.$demo_account->account_id.' بنجاح';
                        $notification->details_ar='تم حذف حساب رقم '.$demo_account->account_id.' بنجاح';

                        $notification->notification_ru='Счет номер '.$demo_account->account_id.' был успешно удален';
                        $notification->details_ru='Счет номер '.$demo_account->account_id.' был успешно удален';
           ...
           if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/demo-accounts');
            }else{
                return redirect('en/spanel/demo-accounts');
            }
    ...
    public function live_accounts()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',999999999)->where('notification_status',0)->get()->all();
        $user=website_admins::where('email',$session_user)->get()->first();
        $accounts=Fx_accounts::where('account_radio_type',1)->orderBy('id','desc')->paginate(10);
        ...
        //         }
            ...
            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | الحسابات الديمو ";
                return view('ar.spanel.live-accounts',compact('title','user','notifications_all','notifications_unseen','accounts','balances','equities','names'));
            }else{
                $title = "Spanel | Live Accounts";
                return view('spanel.live-accounts',compact('title','user','notifications_all','notifications_unseen','accounts','balances','equities','names'));
            }
    ...
    public function referrals_accounts()
    {
        ...
        $demo_accounts=Fx_accounts::where('account_radio_type',0)->get()->all();
            ...
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.referrals-accounts',compact('title','user','notifications_all','notifications_unseen','live_accounts','demo_accounts','ref_accounts','website_accounts'));
            }else{
                $title = "Spanel | My Referrals";
                return view('spanel.referrals-accounts',compact('title','user','notifications_all','notifications_unseen','live_accounts','demo_accounts','ref_accounts','website_accounts'));
            }
    ...
    public function documents()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',999999999)->where('notification_status',0)->get()->all();
        $user=website_admins::where('email',$session_user)->get()->first();
        $documents=documents::orderBy('id','desc')->paginate(25);
        $website_accounts=website_accounts::get()->all();

            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | مستندات المستخدمين ";
                return view('ar.spanel.documents',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts'));
            }else{
                $title = "Spanel | Users Documents";
                return view('spanel.documents',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts'));
            }
    ...
    public function delete_documents($id)
    {
        ...
        $input=Request::all();
                ...
                $notification->notification_status=0;
                $notification->notification='Your Document Has Been Deleted Successfully';
                $notification->notification_ar='تم حذف المستند بجاح';
                $notification->details='Your Document Has Been Deleted';
                $notification->details_ar='تم حذف المستند بجاح';
                $notification->notification_ru='Ваш документ успешно удален';
                $notification->details_ru='Ваш документ успешно удален';

                $notification->notification_link='/cpanel/documents';
                $notification->save();

            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/documents');
            }else{
                return redirect('en/spanel/documents');
            }
    ...
    public function approve_documents($id)
    {
        ...
        $input=Request::all();
                ...
                $notification->website_accounts_id=$website_accounts_id;
                $notification->notification_status=0;
                $notification->notification='Your Pending Document Has Been Approved Successfully';
                $notification->details='Your Pending Document Has Been Approved';
                $notification->notification_ar='تم قبول المستند بجاح';
                $notification->details_ar='تم قبول المستند بجاح';
                $notification->notification_ru='Ваш ожидающий рассмотрения документ был успешно одобрен';
                $notification->details_ru='Ваш ожидающий рассмотрения документ был успешно одобрен';
                $notification->notification_link='/cpanel/documents';
                $notification->save();
        ...
        public function copy_trade()
        {
            ...
            $notifications_unseen=Notifications::where('website_accounts_id',999999999)->where('notification_status',0)->get()->all();
            $user=website_admins::where('email',$session_user)->get()->first();
            $copy_trades=CopyTrade::orderBy('id','desc')->paginate(25);
            $website_accounts=website_accounts::get()->all();

                if( Request::segment(1) =='ar'){
                    $title = "لوحة التحكم | مستندات المستخدمين ";
                    return view('ar.spanel.copy-trade',compact('title','user','notifications_all','notifications_unseen','copy_trades','website_accounts'));
                }else{
                    $title = "Spanel | Users Documents";
                    return view('spanel.copy-trade',compact('title','user','notifications_all','notifications_unseen','copy_trades','website_accounts'));
                }
        ...
        public function delete_copy_trade($id)
        {
            ...
            $input=Request::all();
                        ...
                        $copy_trade=CopyTrade::where('id',$id)->get()->first();
                        $website_accounts_id=$copy_trade->website_accounts_id;
                    ...
                    $notification->website_accounts_id=$website_accounts_id;
                    $notification->notification_status=0;
                    $notification->notification='Your Copy Trade Has Been Deleted Successfully';
                    $notification->notification_ar='تم حذف  نسخ الصفقات الخاصة بك بنجاح';
                    $notification->details='Your Copy Trade Has Been Deleted Successfully';
                    $notification->details_ar='تم حذف  نسخ الصفقات الخاصة بك بنجاح';
                    $notification->notification_ru='Ваша сделка по копированию успешно удалена';
                    $notification->details_ru='Ваша сделка по копированию успешно удалена';

                    $notification->notification_link='/cpanel/copy-trade';
                    $notification->save();

                if( Request::segment(1) =='ar'){
                    return redirect('ar/spanel/copy-trade');
                }else{
                    return redirect('en/spanel/copy-trade');
                }
        ...
        public function approve_copy_trade($id)
        {
            ...
            $input=Request::all();
                    ...
                    $notification->website_accounts_id=$website_accounts_id;
                    $notification->notification_status=0;
                    $notification->notification='Your Pending Copy Trade Has Been Approved Successfully';
                    $notification->details='Your Pending Copy Trade Has Been Approved';
                    $notification->notification_ar='تمت الموافقة على نسخ الصفقات الخاصة بك بنجاح';
                    $notification->details_ar='تمت الموافقة على نسخ الصفقات الخاصة بك بنجاح';
                    $notification->notification_ru='Ваша незавершенная сделка по копированию успешно одобрена';
                    $notification->details_ru='Ваша незавершенная сделка по копированию успешно одобрена';
                    $notification->notification_link='/cpanel/copy-trade';
                    $notification->save();
    ...
    public function deposit_fund_requests()
    {
        ...
        if( Request::segment(4) ==''){
            ...
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.deposit-fund-requests',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts','transactions'));
            }else{
                $title = "Spanel |  Deposit Funds Requests ";
                return view('spanel.deposit-fund-requests',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts','transactions'));
            }
    ...
    public function post_deposit_fund_requests()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',$user->id)->where('notification_status',0)->get()->all();
            ...
            $transaction0=Transactions::where('type',0)->where('id',$input['transaction_id'])->get()->first();


                $notification=new Notifications;
                $notification->website_accounts_id=$website_accounts_id;
                $notification->notification_status=0;
                if($transaction0->status==1){

                  $notification->notification='Your Deposit '.$transaction0->amount.' USD Has Been Done Successfully';
                  $notification->details='Your Deposit '.$transaction0->amount.' USD Has Been Done Successfully';
                  $notification->notification_ar='طلب الأيداع الخاص بك تم بنجاح';
                  $notification->details_ar='تم ايداع مبلغ '.$transaction0->amount.'$ بنجاح';
                  $notification->notification_ru='Ваш депозит '.$transaction0->amount.' долларов США был успешно внесен';
                  $notification->details_ru='Ваш депозит '.$transaction0->amount.' долларов США был успешно внесен';

                }else{

                  $notification->notification='Your Deposit '.$transaction0->amount.' USD Has Been Refused, '.$transaction0->details_user;
                  $notification->details='Your Deposit '.$transaction0->amount.' USD Has Been Refused '.$transaction0->details_user;
                  $notification->notification_ar='تم رفض ايداع مبلغ '.$transaction0->amount.'$ بسسبب'.$transaction0->details_user;
                  $notification->details_ar='تم رفض ايداع مبلغ '.$transaction0->amount.'$ بسسبب'.$transaction0->details_user;
                  $notification->notification_ru='Ваш депозит 999999 долларов США был отклонен, '.$transaction0->details_user;
                  $notification->details_ru='Ваш депозит 999999 долларов США был отклонен, '.$transaction0->details_user;
                }
                ...
                $notification->save();
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/deposit-fund-requests');
            }else{
                return redirect('en/spanel/deposit-fund-requests');
            }
    ...
    public function withdraw_fund_requests()
    {
        ...
        if( Request::segment(4) ==''){
            ...
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.withdraw-fund-requests',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts','transactions'));
            }else{
                $title = "Spanel |  Withdraw Funds Requests";
                return view('spanel.withdraw-fund-requests',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts','transactions'));
            }
    ...
    public function post_withdraw_fund_requests()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',$user->id)->where('notification_status',0)->get()->all();
            ...
            $transaction0=Transactions::where('type',1)->where('id',$input['transaction_id'])->get()->first();
                ...
                if($transaction0->status==1){
                  ...
                  $notification->notification='Your Withdrawal '.$transaction0->amount.' USD Has Been Done Successfully';
                  $notification->details='Your Withdrawal '.$transaction0->amount.' USD Has Been Done Successfully';
                  $notification->notification_ar='تم سحب مبلغ '.$transaction0->amount.'$ بنجاح';
                  $notification->details_ar='تم سحب مبلغ '.$transaction0->amount.'$ بنجاح';
                  $notification->notification_ru='Ваш вывод '.$transaction0->amount.' долларов США был успешно завершен';
                  $notification->details_ru='Ваш вывод '.$transaction0->amount.' долларов США был успешно завершен';

                }else{

                  $notification->notification='Your Withdrawal '.$transaction0->amount.' USD Has Been Refused, '.$transaction0->details_user;
                  $notification->details='Your Withdrawal '.$transaction0->amount.' USD Has Been Refused, '.$transaction0->details_user;
                  $notification->notification_ar='تم رفض سحب مبلغ '.$transaction0->amount.'$ بسسبب'.$transaction0->details_user;
                  $notification->details_ar='تم رفض سحب مبلغ '.$transaction0->amount.'$ بسسبب'.$transaction0->details_user;
                  $notification->notification_ru='Ваш вывод '.$transaction0->amount.' долларов США был отклонен, '.$transaction0->details_user;
                  $notification->details_ru='Ваш вывод '.$transaction0->amount.' долларов США был отклонен, '.$transaction0->details_user;
                }

            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/withdraw-fund-requests');
            }else{
                return redirect('en/spanel/withdraw-fund-requests');
            }
    ...
    public function internal_transfers_requests()
    {
        ...
        if( Request::segment(4) ==''){
            ...
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.internal-transfers-requests',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts','transactions'));
            }else{
                $title = "Spanel |  Internal Transfer Requests";
                return view('spanel.internal-transfers-requests',compact('title','user','notifications_all','notifications_unseen','documents','website_accounts','transactions'));
            }
    ...
    public function post_internal_transfers_requests()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',$user->id)->where('notification_status',0)->get()->all();
           ...
           $transaction0=Transactions::where('type',2)->where('id',$input['transaction_id'])->get()->first();
                ...
                if($transaction0->status==1){
                  ...
                  $notification->notification='Your Internal Transfer '.$transaction0->amount.' USD To '.$transaction0->via.' Has Been Done Successfully';
                  $notification->details='Your Internal Transfer '.$transaction0->amount.' USD To '.$transaction0->via.' Has Been Done Successfully';
                  $notification->notification_ar='تم تحويل مبلغ '.$transaction0->amount.'$ الى '.$transaction0->via.' بنجاح';
                  $notification->details_ar='تم تحويل مبلغ '.$transaction0->amount.'$ الى '.$transaction0->via.' بنجاح';
                  $notification->notification_ru='Ваш внутренний перевод  '.$transaction0->amount.' долларов США на номер '.$transaction0->via.' выполнен успешно';
                  $notification->details_ru='Ваш внутренний перевод  '.$transaction0->amount.' долларов США на номер '.$transaction0->via.' выполнен успешно';
                }else{

                  $notification->notification='Your Internal Transfer '.$transaction0->amount.' USD To '.$transaction0->via.' Has Been Refused '.$transaction0->details_user;
                  $notification->details='Your Internal Transfer '.$transaction0->amount.' USD To '.$transaction0->via.' Has Been Refused '.$transaction0->details_user;
                  $notification->notification_ar='تم رفض طلب التحويل الخاص بك';
                  $notification->details_ar='تم رفض تحويل مبلغ '.$transaction0->amount.'$ الى '.$transaction0->via.' بسسبب'.$transaction0->details_user;

                  $notification->notification_ru='Ваш внутренний перевод '.$transaction0->amount.' USD на '.$transaction0->via.' был отклонен, '.$transaction0->details_user;
                  $notification->details_ru='Ваш внутренний перевод '.$transaction0->amount.' USD на '.$transaction0->via.' был отклонен, '.$transaction0->details_user;
                }
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/internal-transfers-requests');
            }else{
                return redirect('en/spanel/internal-transfers-requests');
            }
   ...
   public function callback_requests()
    {
        ...
        $callbackrequests = callbackrequest::all()->sortByDesc("id");
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.callback-requests',compact('title','user','notifications_all','notifications_unseen','callbackrequests'));
            }else{
                $title = "Spanel | Call Back Request";
                return view('spanel.callback-requests',compact('title','user','notifications_all','notifications_unseen','callbackrequests'));
            }
    ...
    public function delete_callback_requests($id)
    {
        ...
        $callbackrequest->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/callback-requests')->with('status-success','Selected CallBack Has Been Deleted');
            }else{
                return redirect('en/spanel/callback-requests')->with('status-success','Selected CallBack Has Been Deleted');
            }
   ...
   public function landing_users()
    {
        ...
        $notifications_unseen=Notifications::where('website_accounts_id',999999999)->where('notification_status',0)->get()->all();
        $user=website_admins::where('email',$session_user)->get()->first();
        $landingusers = landingusers::orderBy('id','desc')->get()->all();
            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | حسابات الدعايا ";
                return view('ar.spanel.landing-users',compact('title','user','notifications_all','notifications_unseen','landingusers'));
            }else{
                $title = "Spanel | Landing Users";
                return view('spanel.landing-users',compact('title','user','notifications_all','notifications_unseen','landingusers'));
            }
    ...
    public function delete_landing_users($id)
    {
        ...
        $landingusers->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/landing-users')->with('status-success','Selected Landing Has Been Deleted');
            }else{
                return redirect('en/spanel/landing-users')->with('status-success','Selected Landing Has Been Deleted');
            }
   ...
   public function all_search()
    {
        ...
        $user=website_admins::where('email',$session_user)->get()->first();
            ...
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.all-search',compact('title','user','notifications_all','notifications_unseen','all_search'));
            }else{
                $title = "Spanel | All Search";
                return view('spanel.all-search',compact('title','user','notifications_all','notifications_unseen','all_search'));
            }
    ...
    public function delete_search($id)
    {
        ...
        $search->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/all-search')->with('status-success','Selected Search Has Been Deleted');
            }else{
                return redirect('en/spanel/all-search')->with('status-success','Selected Search Has Been Deleted');
            }
    ...
    public function add_search()
    {
        ...
        $input=Request::all();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/all-search')->with('status-success','New Search Has Been Created');
            }else{
                return redirect('en/spanel/all-search')->with('status-success','New Search Has Been Created');
            }
            ...
            public function en_slideshow(Request $request)
            {
                ...
                $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    if( Request::segment(1) =='ar'){
                        ...
                        return view('ar.spanel.en-slideshow',compact('title','user','notifications_all','notifications_unseen','slide1','slide2','slide3','slide4','slide5'));
                    }else{
                        $title = "Spanel | EN SlideShow";
                        return view('spanel.en-slideshow',compact('title','user','notifications_all','notifications_unseen','slide1','slide2','slide3','slide4','slide5'));
                    }
            ...
            public function post_en_slideshow(Request $request)
            {
                ...
                return Redirect::to('/en/spanel/en-slideshow')->with('status-success','SlideShow Has Been Updated');
            ...
            public function ru_slideshow(Request $request)
            {
                ...
                $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    if( Request::segment(1) =='ar'){
                        $title = "لوحة التحكم | أرسال الأيميلات ";
                        return view('ar.spanel.ru-slideshow',compact('title','user','notifications_all','notifications_unseen','slide1','slide2','slide3','slide4','slide5'));
                    }else{
                        $title = "Spanel | EN SlideShow";
                        return view('spanel.ru-slideshow',compact('title','user','notifications_all','notifications_unseen','slide1','slide2','slide3','slide4','slide5'));
                    }
            ...
            public function post_ru_slideshow(Request $request)
            {
                ...
                return Redirect::to('/en/spanel/ru-slideshow')->with('status-success','SlideShow Has Been Updated');
            ...
            public function ar_slideshow(Request $request)
            {
                ...
                $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    if( Request::segment(1) =='ar'){
                        $title = "لوحة التحكم | أرسال الأيميلات ";
                        return view('ar.spanel.ar-slideshow',compact('title','user','notifications_all','notifications_unseen','slide1','slide2','slide3','slide4','slide5'));
                    }else{
                        $title = "Spanel | EN SlideShow";
                        return view('spanel.ar-slideshow',compact('title','user','notifications_all','notifications_unseen','slide1','slide2','slide3','slide4','slide5'));
                    }
            ...
            public function post_ar_slideshow(Request $request)
            {
                ...
                return Redirect::to('/en/spanel/ar-slideshow')->with('status-success','SlideShow Has Been Updated');
            ...
            public function technical_analysis(Request $request)
            {
                ...
                $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    if( Request::segment(1) =='ar'){
                        ...
                        return view('ar.spanel.ar-technical-analysis',compact('title','user','notifications_all','notifications_unseen','posts'));
                    }else{
                      ...
                      $keywords="technical analysis ,MACD indicator, chart patterns, nepse alpha chart, Support and Resistance, Moving Average";
                        return view('spanel.technical-analysis',compact('title','user','notifications_all','notifications_unseen','posts','description','keywords'));
                    }
    ...
    public function delete_technical_analysis($id)
    {
        ...
        $technical->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/technical-analysis?page=1&per=10')->with('status-success','Selected technical Has Been Deleted');
            }else{
                return redirect('en/spanel/technical-analysis?page=1&per=10')->with('status-success','Selected technical Has Been Deleted');
            }
    ...
    public function post_technical_analysis()
    {
        ...
        $technical->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/technical-analysis?page=1&per=10')->with('status-success','New technical Has Been Created');
            }else{
                return redirect('en/spanel/technical-analysis?page=1&per=10')->with('status-success','New technical Has Been Created');
            }
    ...
    public function update_technical_analysis($id)
    {
        ...
        $technical->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/technical-analysis?page=1&per=10')->with('status-success','Selected technical Has Been Updated');
            }else{
                return redirect('en/spanel/technical-analysis?page=1&per=10')->with('status-success','Selected technical Has Been Updated');
            }
        ...
        $technical->save();
            ...
            public function fundamental_analysis(Request $request)
            {
                ...
                $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    if( Request::segment(1) =='ar'){
                        $title = "لوحة التحكم | التحليل الفنى ";
                        return view('ar.spanel.ar-fundamental-analysis',compact('title','user','notifications_all','notifications_unseen','posts'));
                    }else{
                        ...
                        return view('spanel.fundamental-analysis',compact('title','user','notifications_all','notifications_unseen','posts','description','keywords'));
                    }
    ...
    public function delete_fundamental_analysis($id)
    {
        ...
        $fundamental->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/fundamental-analysis?page=1&per=10')->with('status-success','Selected fundamental Has Been Deleted');
            }else{
                return redirect('en/spanel/fundamental-analysis?page=1&per=10')->with('status-success','Selected fundamental Has Been Deleted');
            }
    ...
    public function post_fundamental_analysis()
    {
        ...
        $fundamental->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/fundamental-analysis?page=1&per=10')->with('status-success','New fundamental Has Been Created');
            }else{
                return redirect('en/spanel/fundamental-analysis?page=1&per=10')->with('status-success','New fundamental Has Been Created');
            }
    ...
    public function update_fundamental_analysis($id)
    {
        ...
        $fundamental->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/fundamental-analysis?page=1&per=10')->with('status-success','Selected fundamental Has Been Updated');
            }else{
                return redirect('en/spanel/fundamental-analysis?page=1&per=10')->with('status-success','Selected fundamental Has Been Updated');
            }
        ...
        $fundamental->save();
            ...
            public function news(Request $request)
            {
                ...
                $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    if( Request::segment(1) =='ar'){
                        $title = "لوحة التحكم | التحليل الفنى ";
                        return view('ar.spanel.ar-news',compact('title','user','notifications_all','notifications_unseen','posts'));
                    }else{
                        $title = "Spanel | News";
                        return view('spanel.news',compact('title','user','notifications_all','notifications_unseen','posts'));
                    }
                ...
                public function unionpay_cards()
                {
                  ...
                  $unionpay_cards=unionpay_cards::orderBy('id','desc')->paginate(50);

                  $title = "JMI Brokers | UnionPay Cards";

                  return view('spanel.unionpay-cards',compact('title','user','notifications_all','notifications_unseen','unionpay_cards','website_accounts'));


                }


                        public function delete_unionpay_card($id)
                        {
                            ...
                            $input=Request::all();
                                        ...
                                        $unionpay_cards=unionpay_cards::where('id',$id)->get()->first();
                                        $website_accounts_id=$unionpay_cards->website_accounts_id;

                                        $unionpay_cards->delete();

                                    $notification=new Notifications;
                                    $notification->website_accounts_id=$website_accounts_id;
                                    $notification->notification_status=0;
                                    $notification->notification='Your UnionPay Card Has Been Deleted Successfully';
                                    $notification->notification_ar='تم حذف بطاقة يونيون باى الخاصة بك بنجاح';
                                    $notification->details='Your UnionPay Card Has Been Deleted Successfully';
                                    $notification->details_ar='تم حذف بطاقة يونيون باى الخاصة بك بنجاح';
                                    $notification->notification_ru='Ваша сделка по копированию успешно удалена';
                                    $notification->details_ru='Ваша сделка по копированию успешно удалена';

                                    $notification->notification_link='/cpanel/unionpay-cards';
                                    $notification->save();

                                if( Request::segment(1) =='ar'){
                                    return redirect('ar/spanel/unionpay-card');
                                }else{
                                    return redirect('en/spanel/unionpay-card');
                                }
                        ...
                        public function approve_unionpay_card($id)
                        {
                            ...
                            $input=Request::all();
                                        ...
                                        $unionpay_cards=unionpay_cards::where('id',$id)->get()->first();
                                        $website_accounts_id=$unionpay_cards->website_accounts_id;
                                        $unionpay_cards->status=1;
                                        $unionpay_cards->save();

                                    $notification=new Notifications;
                                    $notification->website_accounts_id=$website_accounts_id;
                                    $notification->notification_status=0;
                                    $notification->notification='Your UnionPay Card Has Been Approved Successfully';
                                    $notification->details='Your Pending UnionPay Card Has Been Approved';
                                    $notification->notification_ar='تمت الموافقة على اصدار كارت يونيو باى الخاص بك';
                                    $notification->details_ar='تمت الموافقة على اصدار كارت يونيو باى الخاص بك';
                                    $notification->notification_ru='Ваша незавершенная сделка по копированию успешно одобрена';
                                    $notification->details_ru='Ваша незавершенная сделка по копированию успешно одобрена';
                                    $notification->notification_link='/cpanel/unionpay-cards';
                                    $notification->save();
    ...
    public function delete_news($id)
    {
        ...
        $news->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/news?page=1&per=10')->with('status-success','Selected news Has Been Deleted');
            }else{
                return redirect('en/spanel/news?page=1&per=10')->with('status-success','Selected news Has Been Deleted');
            }
    ...
    public function post_news()
    {
        ...
        $news->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/news?page=1&per=10')->with('status-success','New news Has Been Created');
            }else{
                return redirect('en/spanel/news?page=1&per=10')->with('status-success','New news Has Been Created');
            }
    ...
    public function update_news($id)
    {
        ...
        $news->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/news?page=1&per=10')->with('status-success','Selected news Has Been Updated');
            }else{
                return redirect('en/spanel/news?page=1&per=10')->with('status-success','Selected news Has Been Updated');
            }
        ...
        $news->save();
            ...
            public function offers(Request $request)
            {
                ...
                $user=website_admins::where('email',$session_user)->get()->first();
                    ...
                    if( Request::segment(1) =='ar'){
                        $title = "لوحة التحكم | التحليل الفنى ";
                        return view('ar.spanel.ar-offers',compact('title','user','notifications_all','notifications_unseen','posts'));
                    }else{
                        $title = "Spanel | offers";
                        return view('spanel.offers',compact('title','user','notifications_all','notifications_unseen','posts'));
                    }
    ...
    public function delete_offers($id)
    {
        ...
        $offers->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/offers?page=1&per=10')->with('status-success','Selected offers Has Been Deleted');
            }else{
                return redirect('en/spanel/offers?page=1&per=10')->with('status-success','Selected offers Has Been Deleted');
            }
    ...
    public function post_offers()
    {
        ...
        $offers->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/offers?page=1&per=10')->with('status-success','New offers Has Been Created');
            }else{
                return redirect('en/spanel/offers?page=1&per=10')->with('status-success','New offers Has Been Created');
            }
    ...
    public function update_offers($id)
    {
        ...
        $offers->save();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/offers?page=1&per=10')->with('status-success','Selected offers Has Been Updated');
            }else{
                return redirect('en/spanel/offers?page=1&per=10')->with('status-success','Selected offers Has Been Updated');
            }
   ...
   public function mailer()
    {
        ...
        $user=website_admins::where('email',$session_user)->get()->first();
            ...
            if( Request::segment(1) =='ar'){
                ...
                return view('ar.spanel.mailer',compact('title','user','notifications_all','notifications_unseen','mails'));
            }else{
                $title = "Spanel | Mailer";
                return view('spanel.mailer',compact('title','user','notifications_all','notifications_unseen','mails'));
            }
    ...
    public function delete_mailer($id)
    {
        ...
        $mail->delete();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/mailer')->with('status-success','Selected Mail Has Been Deleted');
            }else{
                return redirect('en/spanel/mailer')->with('status-success','Selected Mail Has Been Deleted');
            }
    ...
    public function add_mailer()
    {
        ...
        $input=Request::all();
            ...
            if( Request::segment(1) =='ar'){
                return redirect('ar/spanel/mailer')->with('status-success','New Mail Has Been Created');
            }else{
                return redirect('en/spanel/mailer')->with('status-success','New Mail Has Been Created');
            }
   ...
   public function send_mails()
    {
        ...
        $user=website_admins::where('email',$session_user)->get()->first();
            ...
            if( Request::segment(1) =='ar'){
                $title = "لوحة التحكم | أرسال الأيميلات ";
                return view('ar.spanel.send-mails',compact('title','user','notifications_all','notifications_unseen','mails'));
            }else{
                $title = "Spanel | Send Mails";
                return view('spanel.send-mails',compact('title','user','notifications_all','notifications_unseen','mails'));
            }
   ...
   public function post_send_mails________________()
    {
        ...
        //      return $mails;
                ...
                Mail::queue('superadminpanel.mailer.'.$mailtypearr[$mailtype],['title'=>$mails[$mailscount]['title'],'name'=>$mails[$mailscount]['name'],'details' => $maildetails], function($message)use ($mails,$mailscount,$mailsubjectarr,$mailtype){
        ...
        //      return $mails;
                ...
                Mail::queue('superadminpanel.mailer.'.$mailtypearr[$mailtype],['title'=>$mail['title'],'name'=>$mail['name'],'details' => $maildetails], function($message)use ($mails,$mailscount,$mailsubjectarr,$mailtype){
                //  foreach($mails as $mail){
                //      $message->to($mail->mail);
