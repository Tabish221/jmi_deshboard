...
<div class="box box-primary" id="profile-page">
    <div class="box-header">
        <h3 class="box-title"><strong>Fundamental Analysis</strong></h3>
    </div>

    <div class="box-body">
        <h4 class="col-sm-push-1"> All Fundamental</h4>
        <div>
...
<div class="box box-primary">
    <div class="box-header">
        <h3 class="box-title">New Fundamental</h3>
    </div>

    <div class="box-body">
        {!! Form::open(['files'=>true]) !!}

        <label>Fundamental Image</label>
        <input type="file" class="form-control" name="fundamental_image" required />
        ...
        <select class="form-control" name="fundamentalstatus">
...
@if(count($posts)>0)
            ...
            <div class="">
                ...
                <div class="row">          
                    <table class="table table-bordered">
                        <thead>
                            <tr><td colspan="7">
                                    {!! Form::open(['url'=>'en/epanel/fundamental-analysis/','id'=>'DeleteSelected','method'=>'delete','onsubmit' => 'return confirmDelete()','files'=>true]) !!}
                        ...
                        <tbody>
                                ...
                                @foreach($posts as $post)
                                    ...
                                    <tr>
                                        ...
                                        <td>{!! $i !!}</td>
                                        <td>{!! $post->title !!}</td>
                                        <td>@if ($post->fundamentalstatus==1)  <span class="text-success">Published</span>  @endif @if ($post->fundamentalstatus==0) <span class="text-danger">UNPublished</span> @endif</td>
                                        <td>{!! $post->created_at !!}</td>
                                        <td>
                                            <a class="btn btn-success" href="/en/dailyfundamental/fundamental/{!! $post->id !!}" target="_blank">View</a>
                                            <a class="btn btn-success" href="/en/epanel/fundamental-analysis/status/{!! $post->id !!}" >@if ($post->fundamentalstatus==1) Unpublish @endif  @if ($post->fundamentalstatus==0) Publish  @endif</a>
                                        <button type="button" class="btn btn-success" data-toggle="modal" data-target="#edit{!! $post->id !!}" >Edit</button>


                                        {!! Form::open(['url'=>'en/epanel/fundamental-analysis/'.$post->id,'method'=>'delete','onsubmit' => 'return confirmDelete()','style'=>'display:inline;',]) !!}
        ...
        {!! Form::open(['url'=>'en/epanel/update-fundamental-analysis/'.$post->id]) !!}
        ...
        <select class="form-control" name="fundamentalstatus">
            <option value="1" @if ($post->fundamentalstatus==1) selected @endif>Publish Now</option>
            <option value="0" @if ($post->fundamentalstatus==0) selected @endif>Publish Later</option>
        </select>
...
var result = confirm("Are you sure you want to delete this fundamental from fundamental list, You can't undo this?");
...
$('tbody tr td input.checkbox').click(function(){
   ...
   $('#DeleteSelected').attr('action', '/en/epanel/fundamental-analysis/'+Arr);
