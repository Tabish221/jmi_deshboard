...
<div class="box box-primary" id="profile-page">
    <div class="box-header">
        <h3 class="box-title"><strong>news Analysis</strong></h3>
    </div>

    <div class="box-body">
        <h4 class="col-sm-push-1"> All news</h4>
        <div>
...
@if(count($posts)>0)
            ...
            <div class="">
                ...
                <div class="row">          
                    <table class="table table-bordered">
                        <thead>
                            <tr><td colspan="7">
                                    {!! Form::open(['url'=>'en/epanel/news/','id'=>'DeleteSelected','method'=>'delete','onsubmit' => 'return confirmDelete()','files'=>true]) !!}
                        ...
                        <tbody>
                                ...
                                @foreach($posts as $post)
                                    ...
                                    <tr>
                                        ...
                                        <td>{!! $i !!}</td>
                                        <td>{!! $post->title !!}</td>
                                        <td>@if ($post->newstatus==1)  <span class="text-success">Published</span>  @endif @if ($post->newstatus==0) <span class="text-danger">UNPublished</span> @endif</td>
                                        <td>{!! $post->created_at !!}</td>
                                        <td>
                                            <a class="btn btn-success" href="/en/news/{!! $post->id !!}" target="_blank">View</a>
                                            <a class="btn btn-success" href="/en/epanel/news/status/{!! $post->id !!}" >@if ($post->newstatus==1) Unpublish @endif  @if ($post->newstatus==0) Publish  @endif</a>
                                        <button type="button" class="btn btn-success" data-toggle="modal" data-target="#edit{!! $post->id !!}" >Edit</button>


                                        {!! Form::open(['url'=>'en/epanel/news/'.$post->id,'method'=>'delete','onsubmit' => 'return confirmDelete()','style'=>'display:inline;',]) !!}
        ...
        {!! Form::open(['url'=>'en/epanel/update-news/'.$post->id]) !!}
        ...
        <select class="form-control" name="newstatus">
            <option value="1" @if ($post->newstatus==1) selected @endif>Publish Now</option>
            <option value="0" @if ($post->newstatus==0) selected @endif>Publish Later</option>
        </select>
...
var result = confirm("Are you sure you want to delete this news from news list, You can't undo this?");
...
$('tbody tr td input.checkbox').click(function(){
   ...
   $('#DeleteSelected').attr('action', '/en/epanel/news/'+Arr);
